...
const App: React.FC = () => {
  ...
  if (!currentUser) return (
    <div className="h-screen flex items-center justify-center bg-teal-900 p-6">
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans select-none">
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:static inset-y-0 left-0 w-[280px] bg-white border-r flex flex-col z-50 transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="p-4 grid grid-cols-2 gap-2">
          <button onClick={() => setShowCreateModal(true)} className="flex items-center justify-center gap-2 py-3 bg-teal-50 text-teal-700 rounded-xl text-xs font-black hover:bg-teal-100 transition active:scale-95">
            <i className="fa-solid fa-plus-circle"></i> 作成
          </button>
          <button onClick={() => setShowJoinModal(true)} className="flex items-center justify-center gap-2 py-3 bg-slate-100 text-slate-700 rounded-xl text-xs font-black hover:bg-slate-200 transition active:scale-95">
            <i className="fa-solid fa-key"></i> 参加
          </button>
        </div>

        <div className="flex-1 overflow-y-auto px-2 space-y-1">
          <div className="px-4 py-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">チャットルーム</div>
          {rooms.map(room => (
            <div
              key={room.id}
            ...
            >
              <div className="flex items-center gap-3 overflow-hidden">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-black flex-shrink-0 ${activeRoomId === room.id ? 'bg-white/20' : 'bg-teal-100 text-teal-600'}`}>{room.name[0]}</div>
              ...
              {room.createdBy === currentUser.id && (
                <button onClick={(e) => deleteRoom(room.id, e)} className={`p-1 ${activeRoomId === room.id ? 'text-white/60 hover:text-white' : 'opacity-0 group-hover:opacity-100 text-slate-300 hover:text-red-500'}`}>
      ...
      <main className="flex-1 flex flex-col bg-white">
        <header className="h-16 lg:h-20 border-b flex items-center justify-between px-4 lg:px-8 bg-white/90 backdrop-blur-md sticky top-0 z-30 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <h2 className="font-black text-lg text-slate-800 truncate leading-none">{activeRoom?.name || 'ルーム未選択'}</h2>
          </div>
          <button 
            onClick={async () => {
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-6 bg-slate-50/50">
          {!activeRoomId && <div className="h-full flex flex-col items-center justify-center text-slate-300 italic"><i className="fa-solid fa-comments text-5xl mb-4 opacity-10"></i><p>ルームを作成または選択してください</p></div>}
          {messages.map((msg, i) => {
        ...
        <footer className="p-4 sm:p-6 bg-white border-t border-slate-100">
          <form onSubmit={handleSendMessage} className="max-w-4xl mx-auto flex items-end gap-3">
            <button type="button" onClick={() => fileInputRef.current?.click()} className="w-12 h-12 flex items-center justify-center bg-slate-50 text-slate-400 rounded-2xl hover:bg-slate-100 transition-all border border-slate-100"><i className="fa-solid fa-camera text-xl"></i></button>
            <input type="file" ref={fileInputRef} onChange={async (e) => {
              const file = e.target.files?.[0];
              if(file) handleSendMessage(undefined, await processImage(file));
            }} className="hidden" accept="image/*" />
            
            <div className="flex-1 bg-slate-50 rounded-[1.5rem] p-2 border border-slate-200 focus-within:border-teal-400 focus-within:ring-4 focus-within:ring-teal-100 transition-all">
              <textarea
                value={inputText}
              ...
              <div className="flex justify-between items-center px-2 pb-1">
                <button type="button" onClick={() => setIsImportant(!isImportant)} className={`text-[10px] px-3 py-1 rounded-full border-2 font-black transition-all ${isImportant ? 'bg-amber-400 text-white border-amber-500' : 'text-slate-400 bg-white border-slate-200'}`}>重要</button>
              </div>
            </div>
            
            <button type="submit" disabled={!inputText.trim() && !activeRoomId} className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all shadow-lg ${inputText.trim() ? 'bg-teal-600 text-white active:scale-90' : 'bg-slate-100 text-slate-300 shadow-none'}`}><i className="fa-solid fa-paper-plane text-xl"></i></button>
          </form>
        </footer>
      </main>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-sm shadow-2xl animate-in zoom-in duration-300">
            <h3 className="font-black text-xl mb-6 text-slate-800">ルームを作成</h3>
      ...
      {summary && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[200] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            <div className="p-8 bg-teal-600 text-white flex justify-between items-center"><h3 className="font-black text-lg leading-tight"><i className="fa-solid fa-sparkles mr-2"></i>AI 業務要約</h3><button onClick={() => setSummary(null)} className="w-8 h-8 flex items-center justify-center bg-white/10 rounded-full"><i className="fa-solid fa-xmark"></i></button></div>
            <div className="p-8 space-y-6 max-h-[60vh] overflow-y-auto bg-white">
              <div className="bg-teal-50 p-6 rounded-2xl text-sm text-slate-700 italic border-l-4 border-teal-400">{summary.summary}</div>
            ...
            <div className="p-6 bg-slate-50 text-center"><button onClick={()=>setSummary(null)} className="px-12 py-3 bg-white border-2 border-slate-200 rounded-2xl font-black text-slate-500 hover:text-teal-600 transition-all">確認しました</button></div>
          </div>
        </div>
      )}
    </div>
